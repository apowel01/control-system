...
float revolutionsL;
unsigned volatile int counterBandLeft;
...
double Rsmall = 1002;           // Small Resistor of voltage divider
double Rbig = 19750;
...
void setup() {
  ...
  pinMode(BandSensorOutputL, INPUT_PULLUP);
  ...
  attachInterrupt(digitalPinToInterrupt(BandSensorOutputL), tickBand, CHANGE);
...
//  ads1115R.setGain(GAIN_SIXTEEN);  //Sets Full Scale Range to be 0.256V for high resolution
  ...
  CAN.init_Filt(1, 0, 0x150);
  CAN.init_Filt(2, 0, 0x100);
  CAN.init_Filt(3, 0, 0x160);
...
void tickBand() {
  counterBandLeft = counterBandLeft + 1;
}
//----------------------------------------------------------------------------------------------------
